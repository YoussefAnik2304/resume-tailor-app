...
export async function POST(req: Request) {
  try {
    ...
    const { object } = await generateObject({
      ...
      2. Write a 3-sentence profile bridging my background to the JD's specific needs. You MUST explicitly state the exact target job title from the JD in the very first sentence. You MUST naturally weave in missing JD hard skills (like Kanban, BDD, Angular, Gitlab, Maven, etc.). You MUST aggressively wrap key technologies and the job title in **bold** (e.g., "**Concepteur Développeur Java/Angular Senior** with expertise in **Spring Boot**"). If the JD requires a Master's degree, explicitly mention that your Engineering Degree is a Master's equivalent.
